function [x,histout,costdata] = optim_kelley_gradproj(x0,f,up,low,tol,maxit)
//
...
for i=1:ndim
  ...
  kkl(i) = low(i);
	if (kkl(i) > kku(i)) then
    error('gradproj: lower bound exceeds upper bound');
  end
end
...
if (norm(xc - kk_proj(xc,kku,kkl)) > 0) then
  disp('gradproj: initial iterate not feasibile ');
...
while(norm(pgc) > tol & itc <= maxit)
  ...
  while(ft > fgoal)
    ...
    numf   = numf+1;
    if (iarm > 10) then
      disp('gradproj: Armijo error in gradient projection');
