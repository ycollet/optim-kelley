function [x,histout,costdata] = optim_kelley_ntrust(x0,f,tol,maxit,resolution)
//
//
...
while(norm(gc) > tol & itc <= maxit)
  ...
  itc = itc+1;
  [xp,newrad,idid,sdata,nf] = trfix(xc, f, _hess, gc, fc, trrad,ijob,jdata);
  numf = numf+nf;
  ijob = idid;
  if (idid == 2) then
    sdata = jdata;
...
elseif ((ared/pred > .75) & (bflag==1)) then
  trrad = trrad*2;
  idid  = 3;
end
newrad = trrad;
