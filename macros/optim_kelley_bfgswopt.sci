function [x,histout,costdata] = optim_kelley_bfgswopt(x0,f,tol,maxit,hess0)
//
// C. T. Kelley, July 17, 1997
//
// This code comes with no guarantee or warranty of any kind.
//
// function [x,histout] = optim_kelley_bfgswopt(x0,f,tol,maxit,hess0)
//
...
while((norm(gc) > tol) & (itc <= maxit))
  ...
  //
	...
	if (ns==0) then
    dsd = -gc;
    if (userhess==1) then dsd = hess0(dsd); end
  else
    xi   = -dsdp;
    b0   = -1/(y'*s);
...
if (nargin==6) then userhess=1; end

dnewt = dsd; 

if (userhess==1) then dnewt=hess0(dnewt); end
