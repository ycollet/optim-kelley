function [x,lhist,histout] = optim_kelley_imfil(x0,f,budget,scales,parms)
//
//
...
//
// function [x,fcount,histout] = optim_kelley_imfil(x0,f,budget,scales,parms)
//
...
//                 1, 1/2, ... 1/128
//        parms = optional argument = array of conrol parameters
//
//        parms(1) = 
...
if (nargin == 5) then
  np   = length(parms);
  ftol = parms(1);
  if ((np > 1) & (parms(2) == 1)) then fdiff = 1; end
  if (np==3) then quasi = parms(3); end
end
...
while (ns < nscal & fcount <= flim & iquitc < iquit)
  ...
  if ((norm(sgrad,%inf) < stol) | ((sflag+nterm)==0)) then
    //
    ...
    while ((itc < maxit*n) & (fval > ftol) & (norm(sgrad,%inf) >= stol) ...
      ...
      if (sflag+nterm > 0) then
        ...
        //
        if ((itc > 1) & (quasi > 0)) then
          if (quasi==1) then
            _hess = bfupdate(x, xc, sgrad, gc, _hess); 
          else
            _hess = sr1up(x, xc, sgrad, gc, _hess); 
          end
        end
        xc = x;
        //
        ...
        if (quasi > 0) then
          ...
          [fct, x, fval, _hess, iarm] = ...
                  linearm(f, sdir, fval, x, _hess, maxitarm,_beta,h,quasi,fdiff);
          fcount = fcount+fct;
          //
          //     reduce scale upon failure of line search
          //
          if (iarm >= maxitarm) then
            lok  = 0;
            x    = xb;
            fval = fb;
          end
...
function _hess = bfupdate(x, xc, sgrad, gc, _hess)
y = sgrad-gc;
s = x-xc;
z = _hess*s;
if (y'*s > 0) then
  _hess = _hess + (y*y'/(y'*s)) - (z*z'/(s'*z));
end
endfunction
...
function _hess = sr1up(x, xc, sgrad, gc, _hess)
y = sgrad-gc;
s = x-xc;
z = y - _hess*s;
if (z'*s ~=0) then
  ptst = z'*(_hess*z)+(z'*z)*(z'*z)/(z'*s); 
  if (ptst > 0) then _hess = _hess + (z*z')/(z'*s); end
end
endfunction
...
function [fct, x, fval, hessp, iarm] = linearm(f, sdir, fold, xc, _hess, maxitarm,_beta,h,quasi,fdiff)
lambda = 1;
n      = length(xc);
hessp  = _hess;
iarm   = -1;
...
while ((iarm < maxitarm) & (aflag==1))
  ...
  ft   = f(xt);
  fct  = fct+1;
  if ((ft < fval) & (aflag==1)) then
    aflag = 0;
    fval  = ft;
    x     = xt;
  end
  if (aflag==1) then lambda = _beta*lambda; end
end

if ((iarm == maxitarm) & (aflag == 1)) then
