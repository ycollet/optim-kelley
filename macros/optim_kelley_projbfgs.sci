function [x,histout,costdata] = optim_kelley_projbfgs(x0,f,up,low,tol,maxit)
//
...
for i=1:ndim; 
  if (min(kku(i)-xc(i),xc(i)-kkl(i)) < epsilon) then
    alist(i) = 1; 
  end
end
...
_beta         = sstore(:,ns)'*dsd;
dnewt        = dsd-_beta*ystore(:,ns);
ndim         = length(dsd);
xlist        = zeros(ndim,1);
dnewt        = bfgsrp(ystore,sstore,ns-1,dnewt,xlist);
dnewt        = dnewt+(_beta-ystore(:,ns)'*dnewt)*sstore(:,ns); 
